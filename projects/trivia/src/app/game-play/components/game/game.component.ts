...
export class GameComponent implements OnInit, OnDestroy {
  ...
  constructor(private store: Store<AppStore>, private gameActions: GameActions,
    ...
    this.gameObs = store.select(s => s.currentGame).pipe(filter(g => g != null)) ;
    this.gameQuestionObs = store.select(s => s.currentGameQuestion);
  }

  ngOnInit() {
    this.store.pipe(take(1)).subscribe(s => this.user = s.user); //logged in user

    this.route.params
      .subscribe((params: Params) => this.store.dispatch(this.gameActions.loadGame({"gameId": params['id'], "user": this.user})));

    this.store.select(s => s.categoryDictionary).pipe(take(1)).subscribe(c => {this.categoryDictionary = c} );
    this.sub.push(
  ...
  afterAnswer(userAnswerId?: number)
  {
    ...
    timer(1000).pipe(take(1)).subscribe(t => {
      this.continueNext = true;
    });
  }
